...
# parser.add_argument('dataset', choices=['coco', 'voc', 'skyfusion'], required=True, help='dataset type')
...
SKYFUSION_CLASSES = ("Aircraft","ship","vehicle")

ELPD_CLASSES = ("license-plate" , "license plate")

BUILDING_FENCING_CLASSES = (
    "Hanagers",
    "Other",
    "Fenced",
    "Not fenced"
...
if __name__ == '__main__':
    ...
    # elif args.dataset == 'skyfusion':
    #     valid_classes = SKYFUSION_CLASSES
    # else:
    #     raise NotImplementedError

    N = 1#args.n_diff
    # 2. Set your paths
    oracle_path = 'data/Building_Fencing/train/train.json'
    out_root = 'data/Building_Fencing/train'
    
    n_labeled = 5604 # total number of images
    for i in range(N):
        # data_prefix = args.dataset + '_' + args.n_labeled
        generate_active_learning_dataset(
            oracle_json=oracle_path,
            n_labeled_img=n_labeled,
            out_labeled_json=os.path.join(out_root, 'temp_labeled.json'),
            out_unlabeled_json=os.path.join(out_root, 'temp_unlabeled.json'),
            valid_classes=BUILDING_FENCING_CLASSES
        )
