...
def get_inter_feats(lvl_feats, lvl_inds, boxes, img_shape):
    ...
    # Initialize return tensor based on the first feature level's channel dimension
    # Assuming lvl_feats is a list of tensors where each is [B, C, H, W] or [C, H, W]
    sample_feat = lvl_feats[0]
    channel_dim = sample_feat.shape[1] if sample_feat.dim() == 4 else sample_feat.shape[0]
    ret_feats = coor.new_full((coor.shape[0], channel_dim), 0.)
    ...
    for l in range(len(lvl_feats)):
        ...
        # 1. Ensure feat_l is 4D: [1, C, H, W]
        if feat_l.dim() == 3:
            feat_l = feat_l.unsqueeze(0)
        elif feat_l.dim() == 5:
            feat_l = feat_l.squeeze(0) 
        
        # If it's 4D but batch > 1, we take the first item (standard for single-image inference)
        if feat_l.size(0) > 1:
            feat_l = feat_l[0:1]

        # 2. Ensure coor_l is 4D: [1, N, 1, 2]
        # grid_sample expects: [N, H_out, W_out, 2]
        coor_l = coor[mask_l].unsqueeze(0).unsqueeze(2)  # [1, n_det_lvl, 1, 2]

        # 3. Sample
        # Output is [1, C, n_det_lvl, 1]
        inter_feat = F.grid_sample(feat_l, coor_l, mode='bilinear', align_corners=False)
        
        # 4. Reshape back to [n_det_lvl, C]
        inter_feat = inter_feat.flatten(2).squeeze(0).transpose(0, 1)
